...
+┊  ┊29┊import { logout } from './actions/auth.actions';
 ┊27┊30┊
...
@@ -56,6 +59,21 @@
 ...
 ┊58┊61┊
+┊  ┊62┊// middleware for requests
+┊  ┊63┊const middlewareLink = setContext((req, previousContext) => {
+┊  ┊64┊  // get the authentication token from local storage if it exists
+┊  ┊65┊  const { jwt } = store.getState().auth;
+┊  ┊66┊  if (jwt) {
+┊  ┊67┊    return {
+┊  ┊68┊      headers: {
...
+┊   ┊100┊    queryOrMutationLink: middlewareLink.concat(httpLink),
